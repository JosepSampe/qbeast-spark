 ...
 * versa.
 *
 * NOTE: This code is borrowed from Spark 3.2.1 This code is borrowed, so that we can better
 * control compatibility w/in Spark minor branches (3.2.x, 3.1.x, etc)
 *
...
object SchemaConverters {
  ...
  private def toSqlTypeHelper(
      avroSchema: Schema,
      existingRecordNames: Set[String]): SchemaType = {
  ...
  def toAvroType(
    ...
    val schema = catalystType match {
      ...
      case st: StructType =>
        ...
        if (canBeUnion(st)) {
          val nonNullUnionFieldTypes =
            st.map(f => toAvroType(f.dataType, nullable = false, f.name, childNameSpace))
